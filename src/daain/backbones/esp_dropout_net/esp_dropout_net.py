...
from daain.backbones.esp_net.layers import BR, CBR, C, \
    DilatedParllelResidualBlockB, \
    DownSamplerB
from daain.backbones.esp_net.layers import InputProjectionA


class ESPDropoutNetEncoder(nn.Module):
    """
    This class defines the ESPNet-C network in the paper
    """

    def __init__(self, classes: int = 20, p: int = 5, q: int = 3, dropout_rate: float = 0.5) -> None:
        """
        Args:
        ...
        for i in range(p):
            ...
            self.level2.append(nn.Dropout2d(p=self.dropout_rate))
        self.b2 = BR(128 + 3)
        ...
        for i in range(q):
            ...
            self.level3.append(nn.Dropout2d(p=self.dropout_rate))
        self.b3 = BR(256)
...
class ESPDropoutNet(nn.Module):
    """
    This class defines the ESPNet network
    """

    def __init__(self, classes: int = 19, p: int = 2, q: int = 3, encoder: ESPDropoutNetEncoder = None,
        ...
        self.dropout_rate = dropout_rate
        if encoder is None:
            self.encoder = ESPDropoutNetEncoder(classes, p, q)
        else:
        ...
        self.up_l3 = nn.Sequential(
            nn.ConvTranspose2d(classes, classes, 2, stride=2, padding=0, output_padding=0, bias=False), nn.Dropout2d(
                p=self.dropout_rate),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        output0 = self.modules[0](x)  # CBR of input
        inp1 = self.modules[1](x)  # input projection A
        inp2 = self.modules[2](x)  # input projection B

        output0_cat = self.modules[3](torch.cat([output0, inp1], 1))  # BatchNorm over concat
        output1_0 = self.modules[4](output0_cat)  # down-sampled      # Downsampling
...
class CustomESPDropoutNet(nn.Module):
    """A thin wrapper around ESPDropoutNet
    ...
    def __init__(self, device: torch.device, num_classes: int = 20, p: int = 2, q: int = 8) -> None:
        """
        Args:
        ...
        super().__init__()
        self.esp_net = ESPDropoutNet(classes=num_classes, p=p, q=q)
